...
namespace ProceduralLevel.Common.Helper
{
	public class StaticBitMask
	{
		...
		public StaticBitMask(int capacity = INT_SIZE)
		{
			Resize(capacity);
		}
		...
		public bool Contains(StaticBitMask bitMask)
		{
		...
		public bool Equals(StaticBitMask bitMask)
		{
			for(int x = 0; x < m_Mask.Length; x++)
			{
		...
		public void OrMerge(StaticBitMask bitMask)
		{
			for(int x = 0; x < m_Mask.Length; x++)
			{
		...
		public void AndMerge(StaticBitMask bitMask)
		{
			for(int x = 0; x < m_Mask.Length; x++)
			{
				m_Mask[x] = m_Mask[x] & bitMask.m_Mask[x];
		...
		public StaticBitMask Or(StaticBitMask bitMask)
		{
			StaticBitMask mask = Clone();
			mask.OrMerge(bitMask);
			return mask;
		}

		public StaticBitMask And(StaticBitMask bitMask)
		{
			StaticBitMask mask = Clone();
		...
		public StaticBitMask Clone()
		{
			StaticBitMask bitMask = new StaticBitMask(m_Capacity);
			for(int x = 0; x < m_Mask.Length; x++)
			{
